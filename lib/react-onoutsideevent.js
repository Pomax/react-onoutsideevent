...
(function (root, factory) {
  ...
  var onOutsideEventMixin = {
    componentDidMount: function() {
      ...
      // prop exists.
      var evtTypeProp = this.props.listenToOutsideEvent;
      if (!this.props.disableOnOutsideEvent) {
        if (typeof evtTypeProp === 'string' || evtTypeProp instanceof Array) {
          this.enableOnOutsideEvent();
        }
        else {
          throw new Error('Component\'s `listenToOutsideEvent` prop should be a string or an Array.');
        }
      }
    },
